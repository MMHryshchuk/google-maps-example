...
public class MarkerUtils {
    ...
    private static Bitmap getMarkerBitmapFromView(Context context, @DrawableRes int resId, boolean isMe) {
        View marker = LayoutInflater.from(context).inflate(R.layout.marker, null);
        ImageView markerImageView = (ImageView) marker.findViewById(R.id.marker_icon);
        if (isMe){
            markerImageView.setVisibility(View.GONE);
            ImageView location = (ImageView) marker.findViewById(R.id.marker_location);
